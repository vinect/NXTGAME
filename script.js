...
function renderHistory() {
    ...
    if (history.length === 0) {
        ...
        history.slice().reverse().forEach(game => {
            ...
            item.innerHTML = `
                <div><span class="winner-dot ${dotClass}"></span> ${game.p1} : ${game.p2}</div>
                <div style="color:#8E8E93; font-size:0.8rem;">${dateStr}</div>
            `;
            historyList.appendChild(item);
        });
    }

    // Update Stats im Menu
...
function processImage() {
    ...
    // UI Update (Keep Overlay hidden while result is shown)
    document.getElementById('overlay').classList.add('hidden');
    
    // OpenCV logic (Identisch zu vorher)
